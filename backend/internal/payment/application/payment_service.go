...
func (p *paymentService) UpdatePaymentStatus(ctx context.Context, request UpdatePaymentStatusRequest) (resp *UpdatePaymentStatusResponse, err error) {
	...
	if payment.PaymentStatus().IsCompleted() {
		event := eb.NewEvent(
			...
			events.PaymentCompletedPayload{
				PaymentID: payment.ID().String(),
				UserID:    payment.UserID(),
				// In the future, we may support multiple currencies and perform conversions here.
				// The credit system should remain unified and independent of the payment currency.
				// Credit will always be in USD since it's the most common among LLM providers.
				Amount: payment.Amount().Value(),
			},
		)
