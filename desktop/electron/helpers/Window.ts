...
export class Window {
  ...
  private setupAuthHandlers(): void {
    ...
    ipcMain.removeAllListeners("open-auth-url");

    ipcMain.on("open-auth-url", () => {
      console.log("IPC: open-auth-url received");
      this.backend.openAuthUrl();
    });

    console.log("Auth IPC handlers set up");
  }
